...
export class EmailService {
  ...
  async sendEmployeeInvitationEmail(to: string, inviteLink: string, role: "manager" | "cashier", branchName: string): Promise<void> {
    const from = process.env.SMTP_FROM || `"Aura Loyalty" <no-reply@auraloyalty.com>`;
    ...
    const html = `
      ...
      <html>
        <head>
          <meta charset="utf-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Aura Loyalty Personel Davetiyesi</title>
          <style>
            body {
              background-color: #0b0f19;
              color: #f3f4f6;
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              <h1>Personel Davetiyesi</h1>
              <p>Merhaba,</p>
              <p>Aura Loyalty sadakat puanı platformunda şube personeli olarak görev almanız için bir davet aldınız.</p>
    ...
    await this.transporter.sendMail({
      from,
      to,
      subject: `Aura Loyalty - Personel Davetiyeniz (${roleLabel})`,
      html,
    });
  }
}
