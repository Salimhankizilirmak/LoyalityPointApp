...
import { ArrowUpRight, ArrowDownRight, User, Edit3, AlertTriangle } from "lucide-react";
...
const TX_TYPE = {
  ...
  void: { label: "İptal (Void)", color: "#f43f5e", bg: "rgba(244,63,94,0.08)", icon: AlertTriangle },
};
            ...
            return (
              <motion.div 
                ...
                className={`flex items-center gap-4 px-5 py-3.5 group transition-colors ${
                  isVoided
                    ? "bg-rose-950/10 border border-rose-500/20 shadow-[0_0_15px_rgba(244,63,94,0.1)]"
                    : "hover:bg-blue-500/5"
                }`}
              >
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110`} 
                  style={{ background: isVoided ? "rgba(244,63,94,0.08)" : t.bg }}>
                  <TIcon size={16} style={{ color: isVoided ? "#f43f5e" : t.color }} />
                </div>
                ...
                <div className="text-right flex-shrink-0">
                  <p className="text-xs font-black tracking-tight" style={{ color: isVoided ? "#f43f5e" : t.color }}>
                    {tx.pts > 0 && !isVoided ? "+" : ""}{tx.pts} puan
                  </p>
