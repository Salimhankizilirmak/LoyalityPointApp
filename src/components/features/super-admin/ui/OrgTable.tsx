...
interface OrgTableProps {
  ...
  onEditQuota: (org: Org) => void;
}

const fmt = (n: number) => new Intl.NumberFormat("tr-TR").format(n);

export function OrgTable({ orgs, onToggle, onEditQuota }: OrgTableProps) {
              ...
              <motion.tr key={org.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => onEditQuota(org)}
                      className="w-8 h-8 flex items-center justify-center rounded-xl bg-white/5 hover:bg-cyan-500/10 text-slate-400 hover:text-cyan-400 border border-transparent hover:border-cyan-500/20 transition-all active:scale-95"
