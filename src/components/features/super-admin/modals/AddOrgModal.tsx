...
export function AddOrgModal({ onClose, onAdd, isDarkMode = true }: AddOrgModalProps) {
  ...
  const FIELDS = [
    ...
    { key: "slug", label: "URL Slug", placeholder: "Novexitech", icon: GitBranch, mono: true },
    { key: "email", label: "Patron E-postası", placeholder: "novexitech@gmail.com", icon: Mail, mono: false },
  ] as const;
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        className={`w-full max-w-md rounded-3xl overflow-hidden shadow-2xl border transition-colors ${isDarkMode ? "bg-slate-900 border-slate-700" : "bg-white border-slate-100"
          }`}
      >
        <div className={`px-6 py-4 flex items-center justify-between border-b ${isDarkMode ? "border-slate-800" : "border-slate-50"
          }`}>
        ...
        <div className="p-6 space-y-4">
            ...
            <div key={key}>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-4 py-3 rounded-2xl text-sm border outline-none transition-all ${isDarkMode ? "bg-[#0a0f1e] border-slate-700 text-white focus:border-cyan-500" : "bg-slate-50 border-slate-200 focus:border-cyan-400"
                    }`}
          ...
          <div className={`p-4 rounded-2xl flex items-start gap-3 transition-colors ${isDarkMode ? "bg-indigo-500/5 border border-indigo-500/10" : "bg-indigo-50 border border-indigo-100"
            }`}>
          ...
          <button onClick={submit} disabled={!valid || loading || done}
            className={`w-full py-4 rounded-2xl font-bold text-sm flex items-center justify-center gap-2 transition-all shadow-lg shadow-cyan-500/10 ${done
