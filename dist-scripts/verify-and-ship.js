...
async function run() {
    ...
    try {
        ...
        if (addedFiles.length > 0)
            detailParts.push(`Added: [${addedFiles.join(", ")}]`);
        if (modifiedFiles.length > 0)
            detailParts.push(`Modified: [${modifiedFiles.join(", ")}]`);
        if (deletedFiles.length > 0)
            detailParts.push(`Deleted: [${deletedFiles.join(", ")}]`);
        ...
        (0, child_process_1.execSync)(`git commit -F "${tempMsgPath}"`, { stdio: "inherit" });
